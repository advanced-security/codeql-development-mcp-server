---
name: ql-mcp-ext-workshop-author
...
handoffs:
  - agent: ql-mcp-ext-query-developer
    ...
    prompt: 'Develop and test a CodeQL query using TDD methodology. Follow the `ql-mcp-ext-tdd-basic` or `ql-mcp-ext-tdd-advanced` prompt workflow and return the validated query file path and test results when complete.'
    send: false
---

# `ql-mcp-ext-workshop-author` Agent
...
- Follows the bundled `ql-mcp-ext-create-workshop` skill to generate workshops from production queries.
- Validates AST/CFG tools using the bundled `ql-mcp-ext-validate-tools-queries` skill.
...
3. **Validate AST/CFG tools** — use the bundled `ql-mcp-ext-validate-tools-queries` skill to confirm PrintAST, PrintCFG, and CallGraph return non-empty output. **Fail if any query returns empty results.**
...
6. **Generate solution stages** — for each stage, delegate to `ql-mcp-ext-query-developer` (via the **Develop and Test Query** handoff) to create and validate the solution query.
